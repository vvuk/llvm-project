...
/*
 *  @vladv -- the below wasn't right.
 *  it used to define __uint64_t as 'unsigned long', and sys/types.h
 *  would define uint64_t as __uint64.t _if_ stdint.h wasn't included.
 *  If it was, stdint.h would define uint64_t as unsigned long long, which
 *  causes a problem because those are not the same type.
 *
 *  We modify this header to use long long for the 64-bit types everywhere.
 *  On Linux, the 64 bit types typically use "long", so this decision might
 *  bite us.
 *
 */
